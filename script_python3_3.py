...
import random
import smtplib
def get_property_web_scrapping(property_not_to_visit_list):
    """
    ...
    property_not_to_visit_list list of properties that should not be visited
    ...
    for page_no in range(1,2):
        ...
        print("More specifically, page Nr. ",page_no)
...
def get_response_text(url):
    ...
    if response.ok:
        ...
        # xxx THIS NEEDS TO BE FIXED!!! print("\n Show me what is response text: \n" + response_text + " .")
    else:
...
def get_str_list_from_file():
    """
    this function will read file OldPropertyList.txt and returns list containing ids of property present in the file.

    Returns:

    values which are present in OldPropertyList.txt 
    """
    list_values = []
...
def search_keyword(description):
    ...
    """
    key_words_list = ["ocean view\w*", "city view\w*", "island view\w*", "sunset\w*", "sun set\w*", "picturesque\w*", "panoramic", "elevat\w*", "renovat\w*", "handym.n", "handy m.n", "handym.n", "develop\w*"]
    found = False
...
property_not_to_visit_list = get_str_list_from_file()
properties_dict = get_property_web_scrapping(property_not_to_visit_list)
href_list = properties_dict.keys()
search_href_successful_list = []
msg = ""
for href in href_list:
    description = properties_dict[href]
    print(description)
    if search_keyword(str(description)):
